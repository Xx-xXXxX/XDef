...
namespace XDef
{
	internal static class StaticRefHolder
	{
		...
		public static void Unload()
		{
			...
			XDef.Logv1.Debug($"StaticRefHolder Call Unload {Loaded}");
			if (Loaded)
			{
				Loaded = false;
				XDef.Logv1.Debug("StaticRefHolder Unload");
				sRitems = null;
				if(sRs!=null)
				...
				//XDef.Logv1.Debug("StaticRefHolder pre GC.Collect()");
				//GC.Collect(); GC.WaitForPendingFinalizers();
				//XDef.Logv1.Debug("StaticRefHolder post GC.Collect()");
			}
		}
	}
	#pragma warning disable CS1591 // 缺少对公共可见类型或成员的 XML 注释
	/// <summary><![CDATA[
	/// 通过一个弱引用和一个列表引用完成可以在XDef.Unload自动释放的引用
	/// 通过new T()创建对象
		...
		where T : class, new()
	{
		...
		private T CreateItem1()
		{
			...
			XDef.Logv1.Debug($"StaticRefWithNew{this.GetType().FullName} CreateItem1 {V}");
		...
		private T CreateItem2()
		{
			...
			XDef.Logv1.Debug($"StaticRefWithNew{this.GetType().FullName} CreateItem2 {J}");
			StaticRefHolder.SRs.AddLast(Unload);
			return J;
		}
		...
		public StaticRefWithNew(bool CreateNow = false)
		{
			...
			XDef.Logv1.Debug($"StaticRefWithNew{this.GetType().FullName} ctor {((Action)(Unload)).Method}");
		}
		public T Value
		{
			get => CheckItem();
		...
		private void SetItem1(T Item) {
			...
			itemWR = new WeakReference<T>(V);
			SetItemNode(StaticRefHolder.SRitems.AddLast(V));
			XDef.Logv1.Debug($"StaticRefWithNew{this.GetType().FullName} SetItem1 {V}");
			StaticRefHolder.SRs.AddLast(Unload);
		}
		private void SetItem2(T item) {
			...
			SetItemNode(StaticRefHolder.SRitems.AddLast(J));
			XDef.Logv1.Debug($"StaticRefWithNew{this.GetType().FullName} SetItem2 {J}");
		}
		...
		private void Unload()
		{
			...
			XDef.Logv1.Debug($"StaticRefWithNew{this.GetType().FullName} Run Unload");
		}
...
#pragma warning disable CS1591 // 缺少对公共可见类型或成员的 XML 注释
		...
		where T : class
	{
		...
		{
			item = F();
			XDef.Logv1.Debug($"CtorByF{this.GetType().FullName} ctor {item}");
		}
		public T Value
		{
			get => item;
		...
		~CtorByF()
		{
			XDef.Logv1.Debug($"CtorByF{this.GetType().FullName} ~");
			item = null;
		}
	}
#pragma warning restore CS1591 // 缺少对公共可见类型或成员的 XML 注释
...
#pragma warning disable CS1591 // 缺少对公共可见类型或成员的 XML 注释
		...
		where T : class
	{
		...
		private T CreateItem1()
		{
			...
			itemWR = new WeakReference<T>(V);
			SetItemNode(StaticRefHolder.SRitems.AddLast(V));
			XDef.Logv1.Debug($"StaticRefWithFunc{this.GetType().FullName} CreateItem1 {V}");
			StaticRefHolder.SRs.AddLast(Unload);
			return V;
		}
		private T CreateItem2()
		{
			...
			itemWR.SetTarget(J);
			SetItemNode(StaticRefHolder.SRitems.AddLast(J));
			XDef.Logv1.Debug($"StaticRefWithFunc{this.GetType().FullName} CreateItem2 {J}");
			StaticRefHolder.SRs.AddLast(Unload);
			return J;
		}
		public bool ItemAvaliable()
		...
		public StaticRefWithFunc(Func<T> I)
		{
			...
			itemWR = null;
			itemNodeWR = null;
			XDef.Logv1.Debug($"StaticRefWithFunc{this.GetType().FullName} ctor {((Action)(Unload)).Method}");
		}
		public T Value
		{
			get => CheckItem();
		...
		private void Unload()
		{
			itemWR.SetTarget(null);
			itemNodeWR.SetTarget(null);
			XDef.Logv1.Debug($"StaticRefWithFunc{this.GetType().FullName} Run Unload");
		}
